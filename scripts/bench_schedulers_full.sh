...
if [[ -z "$IFACE" ]]; then
  echo "--iface is required for full benchmark report"
  usage
  exit 1
fi
...
echo "iface: $IFACE, ping_target: ${PING_TARGET:-<disabled>}" | tee -a "$LOG"
...
run_case() {
  ...
  if [[ "$sched" == "native" ]]; then
    ...
    echo "native,true,$state,$util,$rx_mbps,$tx_mbps,$rx_irq_delta,$tx_irq_delta,$ctxt_delta,$loss,$avg,$p95,$max" >> "$CSV"
    return 0
  fi
